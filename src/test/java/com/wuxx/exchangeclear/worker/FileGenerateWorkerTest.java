...
class FileGenerateWorkerTest {
    ...
    void handleShouldSkipAlreadyGeneratedTask() {
        ...
        when(settleFileMapper.selectByBiz(task.getSettleDate(), task.getMemberId(), task.getFileType(), task.getVersion()))
                .thenReturn(file(task));

        worker.handle(message(task.getTaskNo()));

        verify(settleFileTaskMapper, never()).updateStatusToGenerating(anyString());
        verifyNoInteractions(fileGenerator, objectStorageService, settlementFileService);
    }
    ...
    void handleShouldGenerateAfterAcquiringSentTask() {
        ...
        when(settlementFileService.saveGeneratedFile(task, generateResult, uploadResult)).thenReturn(file(task));
        ...
        verify(settlementFileService).saveGeneratedFile(task, generateResult, uploadResult);
    }
    ...
    private SettleFile file(SettleFileTask task) {
